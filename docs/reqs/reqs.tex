...
% Sectioning is done by list on the image on slack
% Camera and ML points
\susbsection{Performance}
\begin{enumerate}[{NF.Perf}.1]
	\item The machine learning model shall be supported by FPGA-based hardware acceleration.
	\item The machine learning model shall be implemented for novel video processing applications.
	\item The machine learning model shall have a throughput of at least 2 frames per second.
	\item The machine learning model shall process in real time, with a latency of one second or less.
	\item The machine learning model shall process both live inputs (from the camera) and test inputs (from a pre-recorded source). 
	\item The machine learning model shall output metrics based on objects within a given video frame
	\item The camera shall be capable of focusing on objects at variable distances, between 1 and 30 meters.
	\item The camera shall resolve individual objects from a minimum height of 10 meters to a precision of 10cm across.
	\item The camera shall support video capture at a minimum rate of 10 frames per second.
	\item The camera shall support a minimum resolution of 640x480.
\end{enumerate}

\susbsection{Scalability}
\begin{enumerate}[{NF.Scal}.1]
	\item The cameral shall use a standard interface, for future replacability
\end{enumerate}

\subsection{Capacity}
\begin{enumerate}[{NF.Capa}.1]
	\item .
\end{enumerate}

\subsection{Availability}
\begin{enumerate}[{NF.Avai}.1]
	\item .
\end{enumerate}

\subsection{Reliability}
\begin{enumerate}[{NF.Reli}.1]
	\item .
\end{enumerate}

\subsection{Recoverability}
\begin{enumerate}[{NF.Reco}.1]
	\item .
\end{enumerate}
\subsection{Serviceability}
\begin{enumerate}[{NF.Serv}.1]
	\item .
\end{enumerate}

\subsection{Regulatory}
\begin{enumerate}[{NF.Regu}.1]
...
\begin{enumerate}[{NF.Mana}.1]
	\item .
\end{enumerate}

\subsection{Environmental}
\begin{enumerate}[{NF.Envi}.1]
	\item .
\end{enumerate}

\subsection{Usability}
\begin{enumerate}[{NF.Usab}.1]
	\item .
\end{enumerate}
