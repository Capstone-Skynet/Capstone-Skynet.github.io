...
Each detailed requirement listed in this document is assigned a unique \textit{requirement identifier}, which can be cross-referenced to other project documents (mainly the Design Specification and Validation Specification). 
...
The final system will consist of a \textit{multirotor-mounted computing platform}; which collects, processes, and analyzes aerial video; and a \textit{mobile base station}; which displays the live video feed from the multirotor, overlaid with the results of the computing platform's analysis. The video analysis process is performed in-part through FPGA-based hardware acceleration.

Current products on the market are primarily aimed at consumers and ``prosumers'' -- content-producers whose main focus is aerial photography/videography. While our project also features aerial photography, its emphasis is in performing real-time analytics using the captured video, in addition to supplying custom features and modifications tailored to our client.
...
\begin{enumerate}[label=F.DR.\arabic*, wide=1cm, widest=3cm, leftmargin=*, font=\bfseries, noitemsep,topsep=0pt, parsep=4pt, partopsep=0pt]
	...
	\item The multirotor RPAS must be equipped with lights to indicate the orientation of the RPAS.
	...
	\item The multirotor RPAS must be constructed and reparable using standard, non-proprietary components, fasteners, and connectors.
