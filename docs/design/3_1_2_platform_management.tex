...
With the provided ML accelerator implementation (described in Section \ref{plb_sec}), the delay visible to the PMB between ML request issuance and reply (steps 3 and 4) is approximately two seconds. If the detected objects and/or the multirotor are moving at a sufficient speed, this results in the detection bounding boxes 'trailing' the objects in the live video by approximately two seconds. To resolve this issue, the user may select to configure the system to operate on a "tape-delay" basis. In this configuration, the live video is delayed by an equal amount of time as it takes for the ML results to be generated. While this provides results which are better matched to the video displayed to the user, the added video delay may be disorienting if the video feed is used for flying the multirotor.
