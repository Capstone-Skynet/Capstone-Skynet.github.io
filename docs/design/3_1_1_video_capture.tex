...
The \texttt{raspicam} C++ library is utilized to manage video acquisition from the on-board camera module. \texttt{raspicam} was selected over the native Raspberry Pi camera libraries in order to perform capture and processing in a low-level C++ environment. This allows for required low-level memory manipulation/image format conversions to be performed directly on the (more powerful) PMB, rather than the PLB.
